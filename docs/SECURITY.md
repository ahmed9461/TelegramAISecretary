...
- الصوت والمستندات المدعومة لها حد مستقل، وتُحلل من الذاكرة كبيانات غير موثوقة ولا تُحفظ كمعرفة تلقائيًا.
...
- طلب النجوم مرتبط بمالك ومحادثة وContact ومبلغ وعملة وpayload فريد.
- pre-checkout يتحقق من المستخدم و`XTR` والمبلغ والحالة والصلاحية، ويجيب خلال نافذة Telegram.
- لا تُسلّم الخدمة عند pre-checkout؛ التسليم بعد `successful_payment` مطابق فقط.
- معرف charge من Telegram فريد، وتكرار نفس التأكيد idempotent ولا يرسل تسليمًا ثانيًا.
- السجلات وAudit تحتفظ بالمبلغ والعملة والمعرف الداخلي فقط، ولا تحفظ بيانات بطاقة أو أسرار دفع.
- مفاتيح مزود خارجي لا تدخل `MenuItem.action_config_json` أو Knowledge. زر الرابط الخارجي ليس إثبات دفع ولا ينفذ تسليمًا تلقائيًا.
